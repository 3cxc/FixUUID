...
import static com.github.fixuuid.FixUUID.NoPermissionMessage;
        ...
        if (command.getName().equalsIgnoreCase("whitelist")){
            ...
            if (args.length == 1){
                ...
                if (args[0].equalsIgnoreCase("on")){//开启白名单
                    if (checkPlayerPermission(sender,"fixuuid.whitelist.on",NoPermissionMessage))return false;
                    if (!plugin.getConfig().getBoolean("Whitelist.enable_whitelist")){
                        plugin.getConfig().set("Whitelist.enable_whitelist",true);
                        plugin.saveConfig();
                        sender.sendMessage("已启用白名单");
                        return false;
                    }
                    sender.sendMessage(ChatColor.RED+"白名单已经启用");
                }

                if (args[0].equalsIgnoreCase("off")){//关闭白名单
                    if (checkPlayerPermission(sender,"fixuuid.whitelist.off",NoPermissionMessage))return false;
                    if (plugin.getConfig().getBoolean("Whitelist.enable_whitelist")){
                        plugin.getConfig().set("Whitelist.enable_whitelist",false);
                        plugin.saveConfig();
                        sender.sendMessage("已关闭白名单");
                        return false;
                    }
                    sender.sendMessage(ChatColor.RED+"白名单已经关闭");
                }

                if (args[0].equalsIgnoreCase("reload")){//重载白名单
                    if (checkPlayerPermission(sender,"fixuuid.whitelist.reload",NoPermissionMessage))return false;
                    //重载配置文件
                    plugin.reloadConfig();
                    new ConfigManager().loadConfig();
                    sender.sendMessage("已重新读取配置文件");
                }

                if (args[0].equalsIgnoreCase("list")){//白名单玩家列表
                    if (checkPlayerPermission(sender,"fixuuid.whitelist.list",NoPermissionMessage))return false;
                    if (Player_Whitelist.isEmpty()){
                        sender.sendMessage("白名单内没有玩家");
                        return false;
                    }
                    sender.sendMessage("白名单内共有"+Player_Whitelist.size()+"名玩家: "+Player_Whitelist);
                }
            ...
            if (args.length == 2){
                if (args[0].equalsIgnoreCase("add")){//添加一个玩家到白名单
                    if (checkPlayerPermission(sender,"fixuuid.whitelist.add",NoPermissionMessage))return false;
                    if (!Player_Whitelist.contains(args[1])){
                        Player_Whitelist.add(args[1]);
                        ConfigManager.saveConfig("Whitelist.list",Player_Whitelist);
                        sender.sendMessage("已添加玩家"+args[1]+"到白名单内");
                        return false;
                    }
                ...
                if (args[0].equalsIgnoreCase("remove")){//删除一个玩家的白名单
                    if (checkPlayerPermission(sender,"fixuuid.whitelist.remove",NoPermissionMessage))return false;
                    if (Player_Whitelist.contains(args[1])){
                        Player_Whitelist.remove(args[1]);
                        ConfigManager.saveConfig("Whitelist.list",Player_Whitelist);
                        sender.sendMessage("已从白名单内删除玩家"+args[1]);
                        return false;
                    }
